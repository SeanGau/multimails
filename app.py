...
def index():
    ...
    if flask.request.method == 'POST':
        UPLOAD_FOLDER = os.path.join(path, 'uploads'+str(datetime.now().timestamp()))
        ...
        if csvfile and allowed_file(csvfile.filename):
            ...
            csvfile.save(os.path.join(UPLOAD_FOLDER, filename))
            with open(os.path.join(UPLOAD_FOLDER, csvfile.filename), encoding='utf-8', newline='') as csvfile_saved:
